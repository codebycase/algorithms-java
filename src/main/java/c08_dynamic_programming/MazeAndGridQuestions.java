...
public class MazeAndGridQuestions {
	...
	public boolean findPath(boolean[][] maze, int row, int col, ArrayList<Point> path, HashSet<Point> failedPoints) {
		...
		if (isAtOrigin || findPath(maze, row, col - 1, path, failedPoints)
	...
	public String findShortestWay(int[][] maze, int[] ball, int[] hole) {
		...
		Queue<Point> list = new PriorityQueue<>(); // using priority queue
		...
		while (!list.isEmpty()) {
			...
			for (int i = 0; i < dirs.length; i++) {
				...
				list.offer(new Point(x, y, l, p.s + directions[i]));
			}
		}
