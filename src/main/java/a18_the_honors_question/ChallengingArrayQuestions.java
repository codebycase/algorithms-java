...
public class ChallengingArrayQuestions {
   ...
   * a particular index with more than 1 element along the loop. The loop must be "forward" or
   * "backward'.
   * 
  ...
  /**
   * Design an algorithm that takes a sorted array whose length is not know, and a key, and returns an
  ...
  public static int maxSubarraySumInCircular(List<Integer> A) {
    ...
    return Math.max(findOptimumSubarraySum(A, new MaxComparator()), accumulate - findOptimumSubarraySum(A, new MinComparator()));
  }
