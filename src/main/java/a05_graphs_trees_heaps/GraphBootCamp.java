...
public class GraphBootCamp {
  ...
  private static void markBoundaryRegion(int i, int j, List<List<Character>> board) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (curr.x >= 0 && curr.x < board.size() && curr.y >= 0 && curr.y < board.get(curr.x).size() && board.get(curr.x).get(curr.y) == 'W') {
  ...
  public static void main(String[] args) {
    List<List<Character>> A = Arrays.asList(Arrays.asList('B', 'B', 'B', 'B'), Arrays.asList('W', 'B', 'W', 'B'), Arrays.asList('B', 'W', 'W', 'B'),
        Arrays.asList('B', 'B', 'B', 'B'));
    fillEnclosedRegions(A);
    List<List<Character>> golden = Arrays.asList(Arrays.asList('B', 'B', 'B', 'B'), Arrays.asList('W', 'B', 'B', 'B'),
